...
async def upload_file(background_tasks: BackgroundTasks,
                      file_uploader: Annotated[FileUpload, Depends(get_file_uploader)],
                      structured_llm: Annotated[
                          ChatOpenRouter, Depends(get_llm_client_for_structured_response)
                      ],
    ...
    hash_of_file_contents = FileUpload.calculate_file_hash(file_contents_of_uploaded_file)

    with tempfile.NamedTemporaryFile(delete=False, suffix=".pdf") as temp:
        temp.write(file_contents_of_uploaded_file)
        temp_file_path = temp.name

    document_indexer = DocumentIndexer(structured_llm_instance = structured_llm,
    ...
    file_upload_response = await upload_pdf_file(input_file=input_file,
    ...
    """
    should_schedule_indexing = (
        file_upload_response.upload_status == "Success"
        or (
            file_upload_response.upload_status == "Already Exists"
            and (
                document_indexing_status is None
                or document_indexing_status["status"] == "failed"
            )
        )
    )
...
async def _run_document_indexing_task(document_indexer: DocumentIndexer, document_id: str) -> None:
    try:
        ...
        DocumentIndexer.set_status_for_document_indexing(document_id=document_id,
                                status='failed',
...
@app.get("/index_status/{document_id}/events")
async def index_status_sse(document_id: str) -> StreamingResponse:
...
async def upload_pdf_file(input_file: UploadFile,
                                    file_contents: bytes,
    ...
    file_uploading_status = await file_uploading_task
